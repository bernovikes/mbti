<template>
	<record :data="list" :loadding="loadding" nav="赠送记录">
		<template v-slot:title="{ item }">
			{{ item.answer.question_bank.title }}
		</template>
		<template v-slot:no="{ item }">
			{{ item.answer.order_no }}
		</template>
		<template v-slot:time="{ item }">
			{{ item.created_at }}
		</template>
		<template v-slot:status="{ item }">
			<span :class="{ 'color-b0b0b0': item.redeem_code_use, 'color-fe657a': !item.redeem_code_use }"
				class="f7 fw4  ml-auto">{{ item.redeem_code_use ? '已使用' : '未使用' }}</span>
		</template>
		<template v-if="virtualPayment" v-slot:btn="{ item }">
			<span class="btn white f7 fw5 lh-16" @click="handle(item)">
				{{ !item.redeem_code_use ? '去赠送' : '查看报告' }}
				<span v-if="!item.redeem_code_use" class="icon-arrow dib ml1" />
			</span>
		</template>
...
import { fetchFriendOrder } from '@/api/api.js'
...
const handle = (item) => {
	...
	let data = {}
	if (item.redeem_code_use_order) {
		data['no'] = item.redeem_code_use_order
	} else {
		url = '/pages/send/index'
		data['redeem_code'] = item.redeem_code
		data['id'] = item.answer.question_bank.id
	}
...
onMounted(async () => {
	...
	try {
		const { data } = await fetchFriendOrder(params)
		console.log(data)
